...
# wrap AlgorithmsTest in "empty class" such that it is not discovered by
# test runners which look for classes with unittest.TestCase as a base.
class BaseTestClass:
    class AlgorithmsTest(ConnectionSetup):
        """
        ...
        class TestCalibInverted(TestCalib):
            ...
            def measure_results(self, connection, builder):
                return self.invert(super().measure_results(connection,
                                                           builder))
        ...
        def test_binary_search_not_inverted(self):
            ...
            np.testing.assert_allclose(
                ...
                err_msg="Calibration using Binary Search (non-inverted) "
                        "failed.")
        ...
        def test_linear_search_not_inv(self):
            ...
            np.testing.assert_allclose(
                calibrated_parameters, np.arange(self.n_instances),
                err_msg="Calibration using linear search (non-inverted) "
                        "failed.")
...
class TestAlgorithms16Instances(BaseTestClass.AlgorithmsTest):
    """
...
class TestAlgorithms511Instances(BaseTestClass.AlgorithmsTest):
    """
...
class TestAlgorithms1024Instances(BaseTestClass.AlgorithmsTest):
    """
